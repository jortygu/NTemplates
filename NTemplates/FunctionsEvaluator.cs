...
namespace NTemplates
{
    ...
    internal class FunctionsEvaluator
    {
        ...
        public string Hyperlink(string data,  DataManager manager)
        {
            ...
            string text = manager.GetCurrentValueForPlaceHolder(new CommonMethods(manager.Parser).AddDelimiters(parameters[0].Trim())).ToString();
            string hplnk = manager.GetCurrentValueForPlaceHolder(new CommonMethods(manager.Parser).AddDelimiters(parameters[1].Trim())).ToString();
        ...
        private static string GetStringData(string data, DataManager manager, Match m, TemplateFunctions func)
        {
            ...
            string varNameDelimited = data.Replace(m.ToString(), string.Empty).Replace("(", string.Empty).Replace(")", string.Empty).Replace(",", string.Empty).Replace(GetFunctionName(func), string.Empty);      //.Replace(manager.Parser._d, string.Empty);
            string varName = (new CommonMethods(manager.Parser)).ClearDelimiters(varNameDelimited);

            //TODO: Gonzalo come back and review this!!! We already have the value for the parameter
            string varValue = manager.GetCurrentValueForPlaceHolder(new CommonMethods(manager.Parser).AddDelimiters(varName)).ToString();
            return varValue;
        }
    }
}
